...
function Package() {
    ...
    const [display, setDisplay] = useState(Array(data.length).fill(true));
    
    const updateDisplay = (id) => {
        let updatedDisplay = [...display];
        updatedDisplay[id-1] = false;
        setDisplay(updatedDisplay);
    }

    const isNothingDisplayed = display.reduce((accumulator, currentValue) => accumulator + currentValue, 0) ? false : true;
    
    const handleRefresh = () => {
        setDisplay(Array(data.length).fill(true));
    }

    return (
        <div className="package">
            {data.map((city, index) => 
                display[index] && <Card key={index} id={city.id} name={city.name} info={city.info} image={city.image} price={city.price} updateDisplay={updateDisplay}/>
            )}
            {isNothingDisplayed && 
